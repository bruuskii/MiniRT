...
t_hit *intersect_cone(t_ray *ray, t_cone *cone)
{
    ...
    t_vctr CO = vector_sub(ray->origin, *(cone->vertex));
    t_vctr D = ray->direction;
    t_vctr V = *(cone->axis);
    double k = cone->tang;

    double D_dot_V = dot(D, V);
    double CO_dot_V = dot(CO, V);
    double D_dot_D = dot(D, D);
    double CO_dot_D = dot(CO, D);
    double CO_dot_CO = dot(CO, CO);
    ...
    if (t < 0)
        return NULL;
    t_vctr intersection_point = vec3_add(ray->origin, vector_scale(D, t));
