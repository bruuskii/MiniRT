...
void					put_pixel_to_image(char *img_data, int x, int y,
							int color);
double					dgrs_to_rdn(double angle);
void					convert_3d_to_2d(double x, double y, double z, double d,
							int *x2, int *y2, double fov);
...
void					ft_render_sphere(t_sp **sphere, void *img,
							t_scene *scene);
...
t_ray					create_shadow_ray(t_hit hit, t_vctr point,
							t_light *light);
...
void					ft_assign_alight_utils(t_alight **lt, char **lst,
							int i);
